...
class BusinessAgentRegistry:
    ...
    def __init__(
        self, *, agents: Dict[str, BaseBusinessAgent], agent_selector: AgentSelector
    ):
        ...
        for agent in self._agents.values():
            ...
            if agent:
                ...
                for dependecy in dependencies:
                    ...
                    if tool:
                        logger.info(
                            f"Attach tool {dependecy.tool_name} to Agent {agent.agent_name}"
                        )
                        tools.append(tool)
                    elif dependecy.required:
                        raise FlotillaConfigurationError(
                            f"Agent {agent.get_name} requires Tool {dependecy.tool_name} and it is not available in the ToolRegistry"
                        )
                    else:
                        logger.warn(
                            f"Agent {agent.get_name} requests optinal Tool {dependecy.tool_name} and it is not available in the ToolRegistry"
                        )
    ...
    def select_agent(self, agent_input: AgentInput) -> Optional[BaseBusinessAgent]:
        """
        ...
        return self._agent_selector.select_agent(
            agent_input=agent_input, agents=self._agents
        )
