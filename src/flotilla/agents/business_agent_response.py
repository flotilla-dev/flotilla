...
class BusinessAgentResponse(BaseModel):
    ...
    query: str = Field(
        ..., description="The original user query that triggered the agent"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score from 0.0–1.0"
    )
    message: Optional[str] = Field(None, description="Human-readable summary message")

    reasoning: Optional[str] = Field(
        None, description="Description of how the LLM made its decision"
    )
    ...
    actions: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of next actions the orchestration agent may take"
    )

    # Error list (optional)
    errors: Optional[List[ErrorResponse]] = Field(
        None, description="List of errors that occurred"
    )
