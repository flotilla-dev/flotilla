...
class InMemoryStore(ThreadEntryStore):
    ...
    async def append(
        ...
        async with self._lock:
            # define the entry order with default value of 0
            entry_order = 0
            ...
            # Non-empty thread case
                ...
                entry_order = current_tail.entry_order + 1
                ...
                if entry.previous_entry_id is None:
                    raise AppendConflictError(
                        ...
                        tail_entry_id=current_tail.entry_id,
                        expected_entry_id=expected_previous_entry_id,
                        message=("Previous entry id on TheadEntry cannot be None"),
                    )
                ...
                if current_tail.entry_id != expected_previous_entry_id:
                    raise ConcurrentThreadExecutionError(
                        ...
                        message=(
                            f"Current tail entry_id {current_tail.entry_id} "
                            f"does not match expected tail entry_id {expected_previous_entry_id}"
                        ),
                    )
                if entry.previous_entry_id != current_tail.entry_id:
                    raise ConcurrentThreadExecutionError(
                        ...
                        entry_type=type(entry).__name__,
                        message=(
                            f"entry.previous_entry_id {entry.previous_entry_id} "
                            f"does not match current_tail.entry_id {current_tail.entry_id}"
                        ),
                    )
            ...
            new_entry = entry.model_copy(
                update={"entry_id": str(uuid4()), "timestamp": datetime.now(timezone.utc), "entry_order": entry_order}
            )
